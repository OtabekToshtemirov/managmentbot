...
const app = express();
app.use(bodyParser.json()); // Telegram webhook JSON ma'lumotlarini o'qish uchun

// Telegram bot instance, webhook rejimida
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(chatId, 'Welcome to the Task Manager Bot! Choose an option:', opts);
});

// Webhook endpoint
app.post(`/api/webhook/${process.env.TELEGRAM_BOT_TOKEN}`, (req, res) => {
  bot.processUpdate(req.body); // Telegramdan kelgan yangilanishlarni qayta ishlash
  res.sendStatus(200);
});
